...
export default function Xuke() {
  ...
  const type = location && location.funciton_type
  ...
  const add = () => {
    navigate('addshouquan', {
      state: {
        id: location ? location.xuke.license_id : '传不到咯'
      }
    })
  }
  //请求获取版本
      ...
      <div className='Xuke-mask'>
        <div className='Xuke-register'>
          ...
          <div className='Xuke-fix'>
            <ul>
              <li>软件名：{location ? location.softwarename : ''}</li>
              <li>版本号：{version}</li>
              <li>有效期： {location ? location.xuke ? location.xuke.validity_time : '' : ''}月</li>
              <li>可用功能： {Function}</li>
              <li>生效日期： {moment(location ? location.xuke ? location.xuke.begin_date : '' : '').format('YYYY-MM-DD HH:mm:ss')}</li>
              <li>截止日期：  {moment(location ? location.xuke ? location.xuke.end_date : '' : '').format('YYYY-MM-DD HH:mm:ss')}</li>
              <li>可用授权数量：{location ? location.xuke ? location.xuke.license_num : '' : ''}</li>
            </ul>
